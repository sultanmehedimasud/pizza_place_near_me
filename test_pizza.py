...
class PizzaGame:
    def __init__(self):
        ...
        self.last_time = 0
        self.pizza_cost = 0
        self.current_view = "kitchen"  # Can be "kitchen" or "pizza_making"
    ...
    def start_pizza_making(self):
        ...
        if self.pizza_manager.customer_manager.received_order:
            # Reset any existing pizzas
            if self.pizza_manager.pizza_station.pizza:
                self.pizza_manager.pizza_station.pizza = None
                
            # Create a fresh pizza
            ...
            # Activate pizza making mode
            self.current_view = "pizza_making"
            
            # Reset OpenGL state
            glMatrixMode(GL_PROJECTION)
            glLoadIdentity()
            glOrtho(-900, 900, -350, 450, -1, 1)
            glMatrixMode(GL_MODELVIEW)
            glLoadIdentity()
    ...
    def update(self):
        ...
        if self.current_view == "pizza_making":
    ...
    def render(self):
        ...
        if self.current_view == "pizza_making":  
            ...
            # Draw pizza making interface
            self._draw_pizza_making_background()
            self._draw_pizza_making_toppings_bar()
            self._draw_pizza_making_dough()
            self._draw_pizza_making_instructions()
            self._draw_pizza_making_oven()
            self._draw_pizza_making_pizza_box()
            self._draw_pizza_making_pizza()
        else:
            # Set up 3D view
    ...
    def _draw_pizza_making_background(self):
        """Draw the background for pizza making screen"""
        # Full window background - sky blue color
        ...
        # Draw the platform/table surface
    ...
    def _draw_pizza_making_toppings_bar(self):
        try:
            # Make the bar wider to fit spacing
            self._draw_rect3d(-700, 350, 0, 1100, -750, 10, (0.7, 0.5, 0.3))
            ...
            for topping, pos in topping_positions.items():
                self._draw_rect3d(pos[0] - 45, pos[1] - 45, 0, 90, 90, 10, (0.7, 0.5, 0.3))
                
                if topping == "sauce":
                    self._draw_circle3d(pos[0], pos[1], 10, 40, (1, 0, 0))
                    self._draw_text3d(pos[0] - 30, pos[1] - 70, 20, "Sauce")
                elif topping == "cheese":
                    self._draw_circle3d(pos[0], pos[1], 10, 40, (1, 1, 0))
                    self._draw_text3d(pos[0] - 36, pos[1] - 70, 20, "Cheese")
                elif topping == "sausage":
                    self._draw_circle3d(pos[0], pos[1], 10, 40, (0.6, 0.3, 0))
                    self._draw_rect3d(pos[0] - 25, pos[1] + 20, 12, 50, 5, 3, (0.5, 0.2, 0.1))
                    self._draw_rect3d(pos[0] - 35, pos[1] + 5, 12, 50, 5, 3, (0.5, 0.2, 0.1))
                    self._draw_rect3d(pos[0] - 15, pos[1] - 10, 12, 50, 5, 3, (0.5, 0.2, 0.1))
                    self._draw_rect3d(pos[0] - 25, pos[1] - 25, 12, 50, 5, 3, (0.5, 0.2, 0.1))
                    self._draw_text3d(pos[0] - 39, pos[1] - 70, 20, "Sausage")
                elif topping == "pepperoni":
                    self._draw_circle3d(pos[0], pos[1], 10, 40, (1.0, 0.5, 0.0))
                    offsets = [(-15, 10), (10, 15), (-10, -15), (15, -10)]
                    for dx, dy in offsets:
                        self._draw_circle3d(pos[0] + dx, pos[1] + dy, 12, 9, (0.9, 0.2, 0.2))
                    self._draw_text3d(pos[0] - 47, pos[1] - 70, 20, "Pepperoni")
                elif topping == "onion":
                    self._draw_circle3d(pos[0], pos[1], 10, 40, (1.0, 0.8, 1.0))
                    self._draw_text3d(pos[0] - 18, pos[1] - 70, 20, "Onion")
                elif topping == "olives":
                    self._draw_circle3d(pos[0], pos[1], 10, 40, (0.1, 0.1, 0.1))
                    self._draw_text3d(pos[0] - 27, pos[1] - 70, 20, "Olives")
                elif topping == "oregano":
                    self._draw_circle3d(pos[0], pos[1], 10, 40, (0.0, 0.7, 0.0))
                    self._draw_text3d(pos[0] - 30, pos[1] - 70, 20, "Oregano")
        except Exception as e:
            print(f"Error in _draw_pizza_making_toppings_bar: {e}")

    def _draw_pizza_making_dough(self):
        try:
            # Define positions
            dough_position_display = (-440, -150, 0)
            pizza_position = (-300, 40, 0)
            
            # Draw pizza board below the pizza
            self._draw_rect3d(pizza_position[0] - 60, pizza_position[1] - 60, 5, 120, 120, 10, (0.5, 0.35, 0.18))
            self._draw_text3d(pizza_position[0] - 50, pizza_position[1] - 80, 20, "Pizza Board")
            
            # Draw dough on the left
            self._draw_rect3d(dough_position_display[0] - 70, dough_position_display[1] - 70, 0, 140, 140, 10, (0.5, 0.4, 0.5))
            self._draw_circle3d(dough_position_display[0], dough_position_display[1], 10, 60, (0.9, 0.8, 0.7))
            self._draw_text3d(dough_position_display[0] - 20, dough_position_display[1] - 80, 20, "Dough")
        except Exception as e:
            print(f"Error in _draw_pizza_making_dough: {e}")

    def _draw_pizza_making_instructions(self):
        try:
            instructions = [
                "Press 'D' to add dough",
                "Press 'S' to add sauce",
                "Press 'C' to add cheese",
                "Press 'V' to add vegetables",
                "Press 'M' to add meat",
                "Press 'F' to finish pizza",
                "Press 'B' to box pizza"
            ]
            y_pos = -250
            for line in instructions:
                self._draw_text3d(-650, y_pos, 100, line)
                y_pos += 20
        except Exception as e:
            print(f"Error in _draw_pizza_making_instructions: {e}")

    def _draw_pizza_making_oven(self):
        try:
            oven_position = (300, 0, 0)
            
            # Oven base: dark brown
            self._draw_rect3d(oven_position[0] - 75, oven_position[1] - 75, 0, 150, 150, 60, (0.25, 0.13, 0.05))
            # Oven front: black
            self._draw_rect3d(oven_position[0] - 60, oven_position[1] - 60, 60, 120, 120, 10, (0.0, 0.0, 0.0))
            # Oven door: black
            self._draw_rect3d(oven_position[0] - 40, oven_position[1] - 40, 70, 80, 80, 5, (0.0, 0.0, 0.0))
            # Oven indicator: green
            self._draw_circle3d(oven_position[0], oven_position[1] + 65, 80, 5, (0, 1, 0))
            self._draw_text3d(oven_position[0] - 20, oven_position[1] + 85, 90, "OVEN")
        except Exception as e:
            print(f"Error in _draw_pizza_making_oven: {e}")

    def _draw_pizza_making_pizza_box(self):
        try:
            ...
            # Draw an open box
            # Draw base (bottom)
            self._draw_rect3d(box_position[0] - box_w//2, box_position[1] - box_d//2, base_z, box_w, box_d, base_height, (0.8, 0.6, 0.4))
            # Draw sides
            self._draw_rect3d(box_position[0] - box_w//2, box_position[1] - box_d//2, base_z + base_height, side_thickness, box_d, side_height, (0.7, 0.5, 0.3))
            self._draw_rect3d(box_position[0] + box_w//2 - side_thickness, box_position[1] - box_d//2, base_z + base_height, side_thickness, box_d, side_height, (0.7, 0.5, 0.3))
            self._draw_rect3d(box_position[0] - box_w//2, box_position[1] - box_d//2, base_z + base_height, box_w, side_thickness, side_height, (0.7, 0.5, 0.3))
            self._draw_rect3d(box_position[0] - box_w//2, box_position[1] + box_d//2 - side_thickness, base_z + base_height, box_w, side_thickness, side_height, (0.7, 0.5, 0.3))
            
            # Draw lid
            glPushMatrix()
            glTranslatef(box_position[0], box_position[1] + box_d//2, base_z + base_height + side_height)
            glRotatef(-80, 1, 0, 0)  # Open upwards
            self._draw_rect3d(-box_w/2, -lid_thickness/2, 0, box_w, lid_thickness, lid_length, (0.9, 0.7, 0.5))
            glPopMatrix()
            
            self._draw_text3d(box_position[0] - 30, box_position[1] + 110, base_z + base_height + side_height + 10, "PIZZA BOX")
        except Exception as e:
            print(f"Error in _draw_pizza_making_pizza_box: {e}")

    def _draw_pizza_making_pizza(self):
        try:
            pizza = self.pizza_manager.pizza_station.pizza
            if not pizza:
                return
            ...
            self._draw_circle3d(pizza_position[0], pizza_position[1], 10, 53, (0.9, 0.8, 0.7))
            ...
            for ingredient in pizza.ingredients:
                ...
                if ingredient.name == "sauce":
                    self._draw_circle3d(pizza_position[0], pizza_position[1], z, 49, (0.8, 0.2, 0.1))
                    z += 1
                elif ingredient.name == "cheese":
                    self._draw_circle3d(pizza_position[0], pizza_position[1], z, 47, (1.0, 0.9, 0.4))
                    z += 1
                # Additional ingredients can be drawn here
        except Exception as e:
            print(f"Error in _draw_pizza_making_pizza: {e}")

    def _draw_circle3d(self, x, y, z, radius, color):
        try:
            glColor3f(*color)
            glBegin(GL_POLYGON)
            for i in range(360):
            ...
            print(f"Error in _draw_circle3d: {e}")

    def _draw_rect3d(self, x, y, z, width, height, depth, color):
        try:
            glColor3f(*color)
            ...
            # If depth > 0, extrude for 3D effect
            if depth > 0:
            ...
            print(f"Error in _draw_rect3d: {e}")
    ...
    def _draw_text3d(self, x, y, z, text):
        try:
            ...
            print(f"Error in _draw_text3d: {e}")
...
def keyboard_callback(key, x, y):
    ...
    if game.current_view == "pizza_making":  # Changed from pizza_making_active
        # Handle pizza making mode keys
        if key == b'b' or key == b'B':  # Box and finish pizza
            game.finish_pizza_making_with_box()
        elif key == b'f' or key == b'F':  # Finish pizza without boxing
            if game.pizza_manager.pizza_station.pizza and game.pizza_manager.pizza_station.pizza.is_complete():
                game.finish_pizza_making()
            else:
                print("Pizza is not complete! Needs dough, sauce, and cheese at minimum.")
        # Ingredient additions are handled in the update_pizza_making method
    else:
...
def mouse_callback(button, state, x, y):
    if game.current_view != "pizza_making":
        return
        
    if button == GLUT_LEFT_BUTTON and state == GLUT_DOWN:
        # Convert window coordinates to our coordinate system
        window_height = game.settings.window_height
        converted_x = (x - game.settings.window_width/2) * (1800/game.settings.window_width)
        converted_y = ((window_height - y) - window_height/2) * (800/window_height)
        
        # Check if clicking on ingredients
        topping_positions = {
            "sauce": (-600, 200),
            "cheese": (-470, 200),
            "sausage": (-340, 200),
            ...
            "olives": (50, 200),
            "oregano": (180, 200),
            "dough": (-440, -150)  # Added dough position
        }
        
        # Check each topping position
        for ingredient, (pos_x, pos_y) in topping_positions.items():
            # Use a larger click radius for better usability
            if (abs(converted_x - pos_x) < 50 and 
                abs(converted_y - pos_y) < 50):
                # Add the ingredient
                if ingredient == "dough":
                    if not game.pizza_manager.pizza_station.pizza:
                        game.pizza_manager.pizza_station.create_pizza()
                else:
                    if game.pizza_manager.pizza_station.pizza:
                        new_ingredient = Ingredient(ingredient, 0, 0, 0)
                        game.pizza_manager.pizza_station.add_ingredient(new_ingredient)
                break
        
        # Check if clicking on pizza position to add selected ingredient
        pizza_pos_x, pizza_pos_y = -300, 40
        if (abs(converted_x - pizza_pos_x) < 60 and 
            abs(converted_y - pizza_pos_y) < 60):
            if game.pizza_manager.pizza_station.pizza:
                pass  # Handle placing ingredients on pizza if needed
